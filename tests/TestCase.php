...
namespace Liberu\Ecommerce\Fulfillment\Filament\Tests;

use Filament\Facades\Filament;
use Liberu\Ecommerce\Fulfillment\Filament\Tests\Fixtures\TestPanelProvider;
use Liberu\Ecommerce\Fulfillment\FulfillmentServiceProvider;
 ...
 * notifications, widgets, Livewire, the icon packages — and every one of those is
 * transitive. So this widens the same walk to everything installed, which is what
 * Laravel's own discovery does in an application, and appends the fixture panel.
 */
abstract class TestCase extends PackageTestCase
{
    ...
    /**
     * An actor working in a team, which is the whole of both policies' tenancy.
     *
    ...
    protected function defineEnvironment($app): void
    {
        ...
        $app['config']->set('fulfillment.team_model', TestUser::class);

        // On, so the telemetry assertions are asserting about records that were
        // actually written. Off is the shipped default, because a busy warehouse
        // writes thousands an hour.
        $app['config']->set('fulfillment.telemetry.enabled', true);
    }
